...
namespace Astronaut.Scripts.Monk
{
    ...
    {		
		...
		*/
		...
		static int  AoESleepTime = 250;             // time in ms to delay when moving to avoid AoE
		
		// Healing Related OPTIONS (Use of Potion, Breath of Heaven, Inner Sanctuary, Blinding Flash, and Serenity)
    public int hpPct_UsePotion = 40; 			// Define % of health left to start using potions
    public int hpPct_InnerSanctuary = 50; // Define % of health left to cast Inner Sanctuary
		public int hpPct_BlindingFlash = 80; 	// Define % of health left to cast Blinding Flash
		public int hpPct_Mantra = 50;					// Define % of health left to cast Mantra
		// Serenity OPTIONS
		static bool isUsingPeacefulRepose = true;	// Set this to true if you use the Peaceful Repose rune
		static bool useSerenityOnElites = true;		// Set this to true if you want Serenity used when an elite is the current target
		public int hpPct_Serenity = 70;				// Set the percent of HP you want to cast Serenity on (will still be used even if useSerenityOnElites is true)
		// Breath of Heaven OPTIONS
		public int hpPct_BreathofHeaven = 60;	// Set this to the HP % you want it to cast on to gain HP (set to 0 if you don't want it to cast on HP %)
		...
		public int DistancetoHealthGlobe = 2;		// Set this to how close you want to be to the health globe to pick it up *2 for 0 pickup radius*
		public int hpPct_HealthGlobe = 40;
		static bool DashtoHealthGlobe = false;
		// Blinding Flash OPTIONS
		public int BlindingFlashDistance = 15;
		public int BlindingFlashTargetsNearby = 10;
		// Cyclone Strike OPTIONS
		public int CycloneStrikeDistance = 20;		// Set this to the distance the targets must be within before casting Cyclone Strike (20 for all the runes aside from Implosion, if you are using the Imposion Rune set it to 30)
		public int CycloneStrikeTargetsNearby = 2;	// Set this to the number of targets that must be within CycloneStrikeDistance before casting Cyclone Strike
		...
		static bool DashoutofAOE = false;		// Use Dashing Strike to escape the AOE
		...
		*/
            ...
            try
            {
                while (isWorking)
                {
					...
					if (checkLootTimer.IsReady)
					{
					 ...
					 // Sweeping Wind up
                    if (D3Control.canCast("Sweeping Wind") && D3Control.isMovingWorking() )
                    {
						...
						else if( D3Control.HasBuff("Sweeping Wind") ) 
						{
							CastMonkSpell("Sweeping Wind", D3Control.Player.Location);
						}
        ...
        protected override void DoEnter(D3Player Entity)
        {
            ...
            if (!inited)
            {
                ...
                inited = true;
				// Init Message
				D3Control.output("ASWeiler's Monk v2.0.0 12/25");
				//
                setSpirit();
            }
        }
        ...
        protected override void DoExecute(D3Player Entity)
        {
            ...
            while (true)
            {
				...
				if (D3Control.Player.HpPct <= hpPct_HealthGlobe)
				{
					...
					LastValidHpPct = D3Control.Player.HpPct;
					...
					if (healthGlobe != null)
					{
						...
						else
						{
							do 
							{
								...
								// Cast Serenity
								if (D3Control.canCast("Serenity") && isUsingPeacefulRepose) 
								{
									CastMonkSpell("Serenity", D3Control.Player.Location);
								}
								if (D3Control.canCast("Breath of Heaven"))
								{
									CastMonkSpell("Breath of Heaven", D3Control.Player.Location);
									BreathofHeavenTimer.Reset();
								}
							} while (D3Control.Player.HpPct <= LastValidHpPct && !D3Control.Player.IsDead && D3Control.ClosestHealthGlobe != null);
						}
					}
				}
...
*/		
        ...
        void handleAOE()
        {
			...
			if (D3Control.TargetManager.GetAroundEnemy(CycloneStrikeDistance).Count > 1)
			{
		...
		//
        public void checkMysticAlly(D3Player entity)
        {
        ...
        void doPulling(D3Player entity)
        {
			...
			/*
				Healing Spells
			*/
			// Serenity if we are frozen, feared, etc
			...
			// Serenity based on HP Percent or we are set to use it before attacking an Elite and close to the Elite
			if ((isUsingPeacefulRepose && D3Control.Player.HpPct < hpPct_Serenity) || (target.IsElite && useSerenityOnElites && target.DistanceFromPlayer < 20))
			{
				if (CastMonkSpell("Serenity", D3Control.Player.Location))
				{
			...
			// Breath of Heaven
			if (D3Control.canCast("Breath of Heaven") && (D3Control.Player.HpPct < hpPct_BreathofHeaven || (isUsingBlazingWrath && BreathofHeavenTimer.IsReady)))
			{
				if (CastMonkSpell("Breath of Heaven", D3Control.Player.Location))
				{
					Thread.Sleep(10);
			...
			*/
			...
			else if (D3Control.canCast("Mantra of Conviction") && ((MantraTimer.IsReady && D3Control.HasBuff("Sweeping Wind")) || (!D3Control.HasBuff("Mantra of Conviction"))))
			{
				if (isUsingOverawe)
				{
					...
					else
					{
						...
						if (MobsNearbyforOverawe >= NumofMobsNearbyToUseOverAwe && !OveraweonElitesOnly)
						{
							if (CastMonkSpell("Mantra of Conviction", D3Control.Player.Location))
								MantraTimer.Reset();
							Thread.Sleep(10);
						}
						else if (!D3Control.HasBuff("Mantra of Conviction") )
						{
							if (CastMonkSpell("Mantra of Conviction", D3Control.Player.Location))
								MantraTimer.Reset();
            ...
            if (!D3Control.LOS(target.Location))
            {
				...
				if (!D3Control.HasBuff("Sweeping Wind") && D3Control.canCast("Sweeping Wind"))
				{
					CastMonkSpell("Sweeping Wind", D3Control.Player.Location);
