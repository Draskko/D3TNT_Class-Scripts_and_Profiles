...
namespace Astronaut.Scripts.Monk
{
    ...
    {		
		...
		*/
		...
		static CWSpellTimer MantraTimer = new CWSpellTimer(3 * 1000);	// Meant for all of the Mantra's 3 second "buffs". Set to 3 * 1000 if you want it to cast every 3 seconds. Must have CastMantraEvery3Seconds set to true.
		...
		*/
		...
		void doAttacks()
		{
			...
			if (D3Control.NearbyEnemyCount(35) > 1 || D3Control.curTarget.IsElite)
			{
				...
				// Seven-Sided Strike
				if ((D3Control.NearbyEnemyCount(30) > 5 || D3Control.Player.HpPct < hpPct_SevenSidedStrike) && D3Control.canCast("Seven-Sided Strike"))
				{
